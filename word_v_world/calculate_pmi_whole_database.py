...
def combine_wf_cf_dicts(wf_dict, summed_cf_dict):  # TODO: speed this up, add check if not in wf dict
    wf_cf = {}
    ...
    for w, f in wf_dict.items():
        for pair, cf in summed_cf_dict.items():
            ...
            f2 = 0
            key = (pair[0], pair[1])
            append1 = wf_cf.setdefault(key, []).append((f1, cf))
            append2 = wf_cf.setdefault(key, []).append((f2, cf))
            # print("cf:", cf)
...
def pmi(wf_cf, window_size):
    # pmi = log(cf / (window_size * word_1 * word_2))
    print('Calculating pmi...')
    pmi_form = 'pmi_all_' + datetime.now().strftime('%Y%m%d_%H-%M-%S')
